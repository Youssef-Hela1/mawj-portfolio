...
function saveToStorage(key, value) {
    ...
    savedData[key] = value;
    localStorage.setItem('mawj_mirror_data', JSON.stringify(savedData));
}
    ...
    if (projForm) {
        projForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (editingProjectIndex >= 0) {
                    existingProjects[editingProjectIndex] = newProject;
                } else {
                    existingProjects.push(newProject);
                }

                localStorage.setItem('mawj_mirror_projects', JSON.stringify(existingProjects));
                renderEnhancedProjects();
                closeProjectEditor();
...
function renderEnhancedProjects() {
    ...
    const allProjects = JSON.parse(localStorage.getItem('mawj_mirror_projects') || '[]');

    // Clear container to avoid duplication from script.js
    container.innerHTML = '';
